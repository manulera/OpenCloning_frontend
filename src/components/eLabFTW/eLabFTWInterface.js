...
async function submitSequenceToDatabase({ submissionData: { title, sequenceCategoryId, primerCategoryId }, substate, id }) {
  ...
  try {
    ...
    await uploadTextFileToResource(resourceId, `${title}_history.json`, JSON.stringify({ sources, primers: primersCopy, sequences }), 'history file - generated by OpenCloning');
